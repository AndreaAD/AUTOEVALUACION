...
$datos=array("tipo"=>"una-columna",// (necesario) tamaño del bloque puede ser {una-columna,una-columna-centro,una-columna-centro-medio}
            "titulo"=>"Crear Encuestas - Seleccion de Evidencia", // (no necesario) titulo del bloque
...
<div class="grupo-controles-formulario">
    <div style="display:block;margin-left:2.5em;" class="texto-izquierda">
        <label style="font-weight:bold;font-size:1.3em; padding-right:4em;" for="<?php echo $datos["id"];?>"><?php echo ($datos["label"]);?></label>
    <?php
        $datos_boton = array(
...
<div class="grupo-controles-formulario">
    ...
    <div style="display:block;width:100%;text-align:center;margin-top: 0.2em;" class="controles-formulario">
        <textarea style="width:90%;" placeholder="<?php echo $datos["placeholder"];?>" id="<?php echo $datos["id"];?>" readonly="on"><?php
            if($existenDatos){
                echo ($idCaracteristica.". ".$txCaracteristica);
            }
        ?></textarea>
    </div>
</div>
...
$datos = array(
            ...
            "value"=>"Ver preguntas",//(necesario) valor que mostrar el boton
            "icono"=>"stack", //(necesario) icono que aparecera en el boton, si se desea sin icono poner {none}
            "onclick"=>"enc_verPreguntas(this,'ENC_crearEncuesta_controlador.php');"// (necesario) funcion js que se ejecutara si se hace click en el boton
            );     
