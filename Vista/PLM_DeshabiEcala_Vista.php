...
class DeshabiEscalaView
{
    //se establece un objeto a la vista general 
    //
    function elementos()
    {
    ...
    //se muestran todas las escalas, para luego una ser deshabilitada 
    function mostrar($arrEsca)
    {
        ...
        $datos = array(
                    ...
                    "onclick"=>"buscarDes();"// (necesario) funcion js que se ejecutara si se hace click en el boton
                    );
    ...
    //se muestra una escala en especifico 
    //para ser deshabilitada
    function mostrarEscala($arrEsa)
    {
        ...
        $datos=array(
                    "id"=>"H_opcion", //(no necesario) define el id que tendra el campo
                    "name"=>"H_opcion", // (necesario) define el name que tendra el campo
                    "value"=>"deshabilitar");// (necesario) El atributo value especifica el valor de un elemento    
        $objComponentes->input_hidden($datos);
        ...
        $datos = array(
                    ...
                    "icono"=>"none", //(necesario) icono que aparecera en el boton, si se desea sin icono poner {none}
                    "reset"=>"off",// (no necesario) si se establece reset es para crear un boton que borre el contenido del formulario y el onclick no funcionara
                    "onclick"=>"guardarDes();"// (necesario) funcion js que se ejecutara si se hace click en el boton
                    );
        $objComponentes->button_normal($datos);
